...
| **ROUTER** | Always rebroadcasts, never cancels | **Only SR role allowed in the early window** when coverage remains — takes a window position (half-airtime wide) via `takeRung()`; with nothing unique it takes nothing. Stock ROUTER/REPEATER/ROUTER_CLIENT still get reserved window positions via `isImmediateRelayRouter()` |
| **ROUTER_LATE** | Always rebroadcasts, never cancels | **Not an early relay** - it relays after everyone else by design (`takeLateRung`), so it is neither reserved a position nor given early-window timing. It does count as a node that will not cancel for us (`willNotCancelForUs()`), which is what stands the acknowledgement pass down |
...
**Legacy Node Priority:** SignalRouting reserves early window positions for **stock** ROUTER, REPEATER and ROUTER_CLIENT (`isImmediateRelayRouter()`) because those nodes transmit regardless of SR coordination. An SR node in any of those roles is excluded from the reservation: it publishes topology and is ranked instead. Among ranked SR candidates, **only ROUTER** may take early-window rungs (`takeRung`); CLIENT, CLIENT_BASE, ROUTER_LATE and other SR-active roles use `takeLateRung` past the preset-bound `relay_floor` (`2·CWmax·slot_time`). ROUTER_LATE is deliberately not in the reservation set — relaying late is the whole point of the role — and the predicate is also what admits a stock node as a relay candidate at all, so narrowing it would silently drop candidates as well as reservations. CLIENT_MUTE nodes participate minimally by broadcasting their direct neighbor topology to assist active SignalRouting nodes in making informed unicast routing decisions.
...
2. **SR Broadcasts**: Do not import other nodes' neighbour tables. Broadcast the nodes this radio can hear and those links' SNR/RSSI. Answer an empty boot broadcast from a direct SR-active neighbour so a rebooted peer learns we hear it.
...
Delivery probability is priced from decode margin, not from absolute signal strength. `calculateETX` takes the modem preset in force (the global LoRa config) along with the observed RSSI and SNR. A LoRa frame decodes when SNR clears the demodulator's minimum for the spreading factor in use — a datasheet figure that runs from about -7.5 dB at the fastest preset's spreading factor to about -20 dB at the slowest — so the curve's dominant term is *decode margin*, the reported SNR minus that preset's threshold: deeply negative margin prices a link near the curve's floor regardless of RSSI, margin at the threshold itself prices a link as marginal, and margin a few dB above it saturates the price near its best. RSSI contributes only a second, much narrower term — a mild, monotonic shading for capture effect, interference margin and estimate confidence at very weak absolute signal strength — never enough on its own to move a link across the coverage ceiling except in a narrow band right at the crossover. A reported SNR that is not finite (a corrupted reading) is priced as though it were far below every preset's threshold, not passed through: the curve is total for every (modem preset, RSSI, SNR), including non-finite and out-of-range inputs.

`cfgPoorLinkEtxThreshold` (default 7.0) is unchanged by this: recalibrating the curve changes which (modem preset, RSSI, SNR) triples clear it, which is the point, not the ceiling itself. `SR_OWNER_COST_BUCKET`/`SR_COST_BUCKET_FIXED` and `NeighborGraph`'s own `BIDI_ETX_CEILING` (below) are unchanged too — the new curve's own worst output still sits well below `BIDI_ETX_CEILING`, so it still excludes the same worst-case links from the bidirectional-priority tier. The wire format carries raw RSSI/SNR per neighbour, never ETX, so this is a local scoring change only: old and new firmware keep exchanging topology correctly and disagree only on the price each puts on it, until every node in a branch runs the recalibrated curve. `etxToSignal` is the inverse used only by its own round-trip test — no production path calls it — and, since an ETX alone cannot say how much of it was RSSI and how much was margin, it reports a fixed representative RSSI and recovers SNR as the preset's threshold plus the recovered margin.
...
1. **Direct Neighbor Detection**: When receiving packets directly with signal data (RSSI/SNR), immediate neighbor relationships are established with calculated ETX values. Decode success is not required: `encryptedOk` is set so `handleReceived` runs for undecoded LoRa frames, and a direct header (`isDirectPacket`, not via MQTT) records the sender as an ordinary direct neighbour. Silence for the existing neighbour liveliness window ages the edge out; there is no separate stranger TTL.

2. **Topology Broadcasts**: Periodically broadcast their complete neighbor list using a packed binary format (8 bytes/neighbor) for comprehensive topology learning from other nodes' perspectives. Each entry carries the neighbor's node ID, RSSI, SNR, capability flags, and ETX variance — giving every node the same link stability view. **Transmit path:** a local `directSignals[]` side table (sized to `NEIGHBOR_GRAPH_MAX_EDGES_PER_NODE`, same as direct edge slots) caches the real measured RSSI/SNR from direct RX (`updateNeighborInfo()` only); topology packing reads from that table rather than approximating signal from stored ETX. **Receive path:** mirrored/downstream links remain ETX-only in RAM; merge still uses `calculateETX(reported_rssi, reported_snr)`. The `hop_limit` of every SR topology packet is capped at `cfgBroadcastMaxHops` (default `SR_BROADCAST_MAX_HOPS`), even if the user has configured a higher limit, to bound topology gossip propagation in large networks. **Multi-packet lists:** more than `MAX_SIGNAL_ROUTING_NEIGHBORS` (28) neighbours go out as several chunks of the same topology version, spaced 2× the packet airtime. Header flag bits mark them (`0x02` more chunks follow, `0x04` not the first chunk); a packet with neither flag is a complete list. The authoritative hearsUs override (below) needs the complete list, so receivers gather the listed ids across chunks and run it on the last one; a continuation whose first chunk was missed leaves the flags untouched. The periodic timer is not reset by originated packets: they prove the node is alive but carry no neighbour list. **Version acceptance:** each broadcast carries the sender's u8 topology version. A report is processed when its version repeats or moves forward from the last accepted one by less than 128 (wraparound-safe), when it is the sender's header-only boot broadcast with version 0, direct or relayed (the sender's counter restarted, so the tracked version is reset to 0 and the next neighbour list is version 1; the boot packet itself consumes version 0), when nothing has been accepted from that sender for two broadcast intervals (`2 × topology_broadcast_secs`, any version is taken as the new base), when the received version sits more than 7 counts behind the last accepted one (a restarted counter such as 26→1, not a delayed copy of v=25; logged `counter restarted`), when the boot broadcast was lost and rejected reports then climb in the new epoch (v=1 then v=2, or v=1 then v=3 if one list was missed; delayed old reports sit 1–2 behind `last` and do not climb), or when a **complete neighbour list is heard from the originator** (`isDirectPacket`) even though the counter looks slightly backwards — the neighbour is transmitting now, so that list replaces what we stored; a relayed copy of a near-behind version stays stale so a delayed old report cannot clobber a newer one. Ingest uses the same gate in `handleReceivedProtobuf` as in `preProcessSignalRoutingPacket`, so a node that stock-floods instead of SR-relaying still records stale rejects and still rebases. These rules apply a rebooted neighbour's list without waiting for that neighbour's counter to catch up, and without rebooting the receiver; each such re-base is logged as `Topology version resync`. Anything else is logged as `Ignoring stale topology` and does not touch the graph.

3. **Relayed Packet Inference**: When receiving a relayed packet, gateway relationships are inferred between the original sender and the relay node. If the relay node is a stock (Legacy) firmware node, a directed edge is also recorded from the relay to the sender — observing a successful relay proves the relay can hear the sender, regardless of the sender's firmware type. The reverse edge is not assumed. Both are priced at a nominal link (RSSI −70, SNR 5), never at the measured strength of the relay's own link to us: a downstream entry stands for a path and is priced per hop the packet travelled, while an inferred edge stands for the single hop from the relay to the sender and is priced as one, whatever route the packet took to reach us.
   **Travelling neighbour:** a sender we still listed as a Reported direct neighbour, heard only
...
1. **Direct Neighbor Detection**: Only neighbors heard directly (`isDirectPacket()` — hopStart == hopLimit and relay_node matches sender, with signal data) are tracked with measured ETX values. Same undecoded-admission rule as active nodes: a heard-but-undecrypted direct frame still enters the Level 1 set.
...
If the packet carries a `next_hop` field, slot 0 is reserved for that designated node. Any SR node whose last-byte matches `p->next_hop` takes slot 0 itself, at the earliest floor in force rather than at once. All other SR nodes start their candidate ranking from slot 1, after a reservation long enough for the designated node's relay to have left the air. The two cases start their clocks in different places: an **SR** next hop obeys the same floor model, so it keys up at the floor and then needs its own contention delay at the current channel utilization plus one airtime *on top of* it; a **stock or unknown** next hop knows nothing of the floor and starts contending the moment it hears the frame, so its copy is clear after the worst-case stock CLIENT delay plus one airtime, with the floor applying only as a lower bound. Adding the floor to the stock case would count the same silence twice. If `next_hop` is not set (`NO_NEXT_HOP_PREFERENCE`), SR candidates start from slot 0, which waits stock's contention floor (`getRelayFloorMsec()`) — or the destination's ACK wait where that is longer — rather than keying up at once.
...
**Slot timing:** slots are ordinal, and every wait is a floor measured from the frame we just heard, so the floors compose by max and never by addition (`srUnicastSlotDelayMs()`). Slot 0 waits stock's own contention floor, `getRelayFloorMsec()` — below it we would key up while a stock neighbour is still inside its own contention window, so it could not have heard our copy, would not cancel its own, and the duplicate we were avoiding would happen anyway — or the destination's ACK wait where that is longer. Every later slot takes the larger of that floor and the leader wait (`SR_PEER_TURNAROUND_MS` plus the maximum contention delay at the current channel utilization plus one airtime), and only then do slots space out by half an airtime: folding the spacing into the comparison instead of adding it afterwards would drop two adjacent rungs onto one millisecond. Behind a designated next hop the ranked candidates follow its slot-0 reservation, which starts from the same floor. A strictly positive jitter, 1 to `max(halfAirtime / 2, SR_MIN_TIE_BREAK_RANGE_MS)` ms, keeps equal slots apart; it is never signed, so a slot cannot be pulled in front of its own position.

Two of those branches were unreachable until 2026-09-11: the reservation base was seeded with the floor rather than left at zero, so it was never zero, every candidate took the reservation path, ranked slot 0 never saw the contention floor and the leader wait was computed and discarded. Test: `test_unicast_slot_waits_are_floors_not_addends`. (The earlier ETX-gap formula masked the tier bit off the costs, so a downstream-tier leader made every gap zero and a node ranked last fired at 0 ms.)
...
0. **Pre-coverage**: nodes that can hear heardFrom are marked already covered, which `NeighborGraph::covers()` decides. The set is taken from other nodes' published graphs (who listed heardFrom), not from heardFrom's own RX list. The evidence it demands depends on whether the receiver ever reports:

   - **It publishes topology** (SR active or passive): it is held to its own lists, so the delivery direction must be evidenced (`knownToHear` — `hearsUs` on heardFrom's edge, or the receiver listing heardFrom with a *measured* edge). An Inferred reverse is the symmetry guess written when someone heard the receiver, and is not evidence the receiver hears them. An edge alone is one-directional: heardFrom listing a node only says heardFrom hears it, and a reporting node's silence about heardFrom is itself information.
   - **It publishes nothing** (stock, or not yet classified): it can never confirm anything, so heardFrom's own edge to it is all the evidence there will ever be, and that edge counts. Holding this class to the reporting standard meant no transmitter could ever cover such a node, and every neighbour of one silent Alert node relayed every frame on the branch.

   Either way the delivery-direction cost, priced at the receiver (`hopCost`), must stay below the configured poor-link ETX (default 7.0, which excludes the curve's own floor, its "heard once" case): `hearsUs` is sticky, so a peer that heard the transmitter once keeps the flag long after its link decayed — a rooftop node kept it with its antenna 20 dB down. The same rule prices a candidate's coverage set, copies already heard, and `uniqueCoverageNeighbor()`. Planned later slots do not absorb.

   **Ownership.** A neighbour that no transmitter can be shown to reach still deserves one relay, but only one: `coverageOwner()` picks it, and both the slot ranking and unique coverage consult it. Admission and absorb share one predicate, `admitsCoverage()` — covers, or ownership naming that relay — because crediting only `covers()` on absorb left an owned neighbour uncovered after its owner transmitted, so a later rung relayed for it again. Absorb runs only for a copy already on the air. Two questions, two rules: "will this relay carry the node if it transmits" is `admitsCoverage()`, while "did a transmission that already happened reach the node" — pre-coverage of the source and the node we heard it from, and the dupe path's view of everyone it heard transmit — is `covers()` alone, because ownership says who *would* carry a node and is no evidence that a frame already on the air got there. Only a silent neighbour has an owner: one that publishes topology and never lists us is nobody's coverage and gets no relay from anyone, deliberately, because every node's own report says none of us reaches it. Since such a node never reports, its receive path is all anyone can measure, so the owner is the node hearing it best — stock ROUTER/REPEATER/ROUTER_CLIENT given way first (they rebroadcast regardless of SR and already hold the earliest slots), then the best link in `SR_OWNER_COST_BUCKET` buckets, then the lowest node id. Mute and passive nodes never own, because they never relay. **Only a silent neighbour has an owner**: one that publishes topology and does not list a candidate has reported that the candidate cannot reach it, and that silence is evidence, so nobody owns it. **Ownership stops at the coverage ceiling**: a link past `cfgPoorLinkEtxThreshold` (the curve's own floor included) delivers nothing, so its holder owns nothing and the neighbour is simply out of reach. Ownership decides *who* carries such a neighbour, never *whether* it is reachable — without that bound the ranking credited unique coverage to a node that cannot deliver and gave it the first slot, so the packet waited a full defer window for a relay that could not come. Measured 2026-09-08: 74 of 183 slots went out over links worse than the ceiling, and the branch's insurance fired to carry those packets instead. The separate stock-coverage phase that used to relay for a mute or legacy neighbour "nobody ranked ahead covers" is gone, together with its own owner election and legacy-role predicates. It applied no ETX ceiling and iterated the relay's Mirrored edges as well, so it relayed in exactly the states the coverage model calls undeliverable — a link past the ceiling, one at the heard-once sentinel, a neighbour never measured directly — while where the link is sound the ranking already carries it, because coverage admits a silent neighbour on the relay's own edge. A deliberate narrowing, not a no-op. The dupe path's third election went with it — one rule decides whose neighbour it is. `uniqueCoverageNeighbor()` returns the neighbour rather than a bool and logs `Relaying for %08x (no transmitter reaches it)`: a relay nobody needs and a relay that saves a node are otherwise indistinguishable in a field log.
...
3. **SR candidate ranking**: Remaining SR-active candidates with unique coverage versus the transmitter (and copies already heard) all get a slot. **SR ROUTER** candidates are placed in the early window first, even if a CLIENT or CLIENT_BASE uniquely reaches more neighbours; every other SR-active role is placed on the late ladder. Within a window, `findBestRelayCandidate` orders by role, then bidi tier, unique count, delivery-direction ETX, then packet-ID-parity node ID. If it's us, we schedule TX and stop. If a candidate already transmitted, we absorb its coverage without consuming a slot. A peer merely ranked into an earlier slot is **not** assumed to have covered anything: overlapping candidates all schedule, and a heard dupe is what subtracts coverage and cancels. A candidate's coverage set is who can hear it: publishers that listed it (measured reverse edge), or silent neighbours it owns. Ranking ourselves and ranking a peer use the same predicate, so colocated nodes agree on the order.
...
A neighbour proves it hears us in three ways, and they share one definition (`confirmSenderHearsUs()`, which reports whether the flag actually changed): its topology list names us, we watch it carry a frame of ours, or a frame of its own reaches us **direct** and names our byte as its next hop. The third holds because a next hop is learned from traffic received — a peer could only have chosen to route through us by hearing us. It must be a direct frame, since on a relayed one that byte was stamped by the relayer and says nothing about the originator, and never a broadcast, where the field carries no designation. Without it, a reply to a neighbour whose list has not arrived yet cannot be framed as a last hop and goes out with a spare hop for others to relay.
...
An empty topology broadcast from a direct SR-active neighbour is a bootstrap request (a node that just booted). Every role that can send topology answers it with a jittered re-broadcast of its own list, passive roles included: they never process topology, so the request is detected in `handleReceived()` rather than in `preProcessSignalRoutingPacket()`. Without this a rebooted neighbour learned that a passive node hears it only at that node's next periodic broadcast. Replies are rate-limited to one per `BOOTSTRAP_REPLY_MIN_INTERVAL_MS` (120 s) per node, so a burst of empty broadcasts cannot make every node flood the channel with its list; a requester inside the cooldown is already in our graph and is covered by the next periodic broadcast. The request also records the time it arrived, and the queued reply is dropped if a broadcast carrying neighbours went out after that time: the requester already has what it asked for. The reply is deliberately not gated on the periodic interval, so that a booting neighbour never waits it out; this rule is what keeps a periodic list and a queued reply from leaving back to back under consecutive versions, which every receiver accepts as two fresh reports and every relayer duplicates. Our own header-only boot broadcast does not count as an answer: it names no neighbours.
...
1. **Pre-coverage**: heardFrom's neighbors with ETX < 7.0 marked as already covered; ETX ≥ 7.0 (below the modem preset's decode threshold, or otherwise priced hopeless) excluded
...
4. **SR Candidate Ranking**: SR ROUTER candidates with unique coverage take early-window rungs first, even if a non-ROUTER uniquely reaches more neighbours. Other SR-active roles with unique coverage take late rungs. Within a window, pick best by role, bidi tier, unique count, ETX, packet-ID-parity node ID; assign the next slot; remove from candidates. Do not absorb a peer that has not transmitted yet — overlapping unique coverage still earns a later slot.
5. **Self-Assignment**: When we're picked as best candidate, schedule TX at that slot's delay
6. **Already-Transmitted Absorption**: Candidates that already transmitted have their coverage absorbed without consuming a slot
7. **Forced Relay**: If not assigned a slot, a downstream override may still force a relay
8. **Dupe Suppression**: If a relay arrives before our slot fires, unique-coverage cancel subtracts that relayer (and every copy heard so far). Nothing unique left → cancel; still unique → keep the queued TX.
