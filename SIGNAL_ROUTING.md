...
| **ROUTER** | Always rebroadcasts, never cancels | **Ranked early when coverage remains** - an SR ROUTER with unique coverage takes a window position (half-airtime wide) ahead of non-ROUTER peers of equal coverage; with nothing unique it takes nothing. Stock ROUTER/REPEATER/ROUTER_CLIENT still get reserved window positions via `isImmediateRelayRouter()` |
...
**Legacy Node Priority:** SignalRouting reserves early window positions for **stock** ROUTER, REPEATER and ROUTER_CLIENT (`isImmediateRelayRouter()`) because those nodes transmit regardless of SR coordination. An SR node in any of those roles is excluded from the reservation: it publishes topology and is ranked instead. ROUTER_LATE is deliberately not in the reservation set — relaying late is the whole point of the role — and the predicate is also what admits a stock node as a relay candidate at all, so narrowing it would silently drop candidates as well as reservations. Ranked positions (including an SR ROUTER that earned coverage) take a half-airtime inside the window while room remains, then spill past the transition. CLIENT_MUTE nodes participate minimally by broadcasting their direct neighbor topology to assist active SignalRouting nodes in making informed unicast routing decisions.
