...
- **SR broadcasts firing at `cfgDirtyBroadcastSecs` rate instead of `cfgBroadcastSecs`**: means topology is continuously dirty. Check `updateNeighborInfo` for repeated significant ETX changes on a direct neighbor (`etxChangeThreshold`) caused by radio link instability, or a direct neighbor cycling in/out of range.
