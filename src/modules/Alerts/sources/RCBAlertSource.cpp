                                     ...
                                     int maxLocationChars) const
{
    ...
    String prompt = "Jesteś parserem alertów w języku polskim. Zwróć TYLKO ten format (bez dodatkowego tekstu):\n\n"
                    "message:{message}|||___|||where:{where}|||___|||severity:{severity}|||___|||start:{start}|||___|||end:{end}\n\n"
                    "ZASADY:\n\n"
                    "{message}:\n"
                    "- Wyciągnij fakty z tekstu (max " + String(maxMessageBytes) + " bajtów UTF-8).\n"
                    "- PRZEPISZ dokładnie: rodzaj zagrożenia, lokalizację, daty/godziny (jeśli są!)\n"
                    ...
                    "\n"
                    "{where}:\n"
                    "- Nazwa geograficzna z TITLE i INTRO (max " + String(maxLocationChars) + " znaków).\n"
                    ...
                    "- Tylko data? → {start} = data 00:00:00, {end} = data 23:59:59\n"
                    "\n"
                    "{severity}:\n"
                    "- Liczba 0-10. "
                    ...
                    "message:Opady śniegu 22-23.11. Możliwe utrudnienia. Zachowaj ostrożność na drogach i chodnikach. Śledź komunikaty pogodowe.|||___|||where:woj. małopolskie|||___|||severity:7|||___|||start:2025-11-22 00:00:00|||___|||end:2025-11-23 23:59:59\n\n"
